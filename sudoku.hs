...
deadEnds s = do
  allDeadEnds <- gets deadEndsS
  let dEnds = filterMap (\(DeadEnd sq i _) -> if sq == s then Just i else Nothing) allDeadEnds
  return dEnds
  ...
  if continue
    then if null probs
         then do solver''' <- oneStepIO solver''
                 ...
                 solveIO solver'''
         else do let solver''' = execState (markDeadEnd >> undoLastMove) solver'' -- mapM_ (putStrLn . display) probs
                 solveIO solver'''
...
recordMove m = modify (\s@Solver{ movesS = moves } -> s { movesS = (m:moves) })
...
markDeadEnd = do 
  ...
  put $ s { deadEndsS = (de: deadEndsS s) }
...
main = do 
  let game = initialState seedValue
  putStrLn ("Seed is: " ++ show seedValue)      
  args <- getArgs 
  solved <- case args of 
    ("-v":_) -> solveIO game
    _ -> return $ execState solve game
